...
def process_raster(geo_file, roi_file, target_resolution, output_geo):
    ...
    with rasterio.open(geo_file) as src:
        ...
        # ---- Buffer de 10 km (en metros, reproyectando a UTM) ----
        utm_crs = roi.estimate_utm_crs()
        roi_utm = roi.to_crs(utm_crs)
        roi_buffered_utm = roi_utm.buffer(100000)  # 100 km
    ...
    with rasterio.open(cropped_raster) as dest:
        band = dest.read(1).astype("float32")  # permitir NaNs temporalmente
        ...
        with rasterio.open(reclassified_raster, "w", **reclassified_meta) as final:
    ...
    # ---- Crear geology_classdefinition.txt ----
    n_units = len(val_map)
...
geo_file = "C:/Users/rappe/OneDrive/Documentos/FONDECYT_CAMILA/mhm_snow/DATA/RAST/Geology/glim_wgs84_0point5deg.txt.asc"
roi_file = "C:/Users/rappe/OneDrive/Documentos/FONDECYT_CAMILA/mhm_snow/DATA/SHP/Cuencas/cuencas_grupos/grupo6.geojson"
target_res = config["cellsize"]
morph_folder = "C:/Users/rappe/OneDrive/Documentos/FONDECYT_CAMILA/mhm_snow/PROC/morph"
# Ejemplo de uso:
