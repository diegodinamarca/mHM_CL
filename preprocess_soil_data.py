...
def read_clip_and_resample_raster(raster_path, shapes, dem_path, resampling_method):
    ...
    with rasterio.open(raster_path) as src:
        ...
        # 1. Clip to ROI
        clipped, clipped_transform = rasterio.mask.mask(src, shapes, crop=True, nodata=nodata_val)
        ...
        # 2. Reproject to match DEM shape, transform, and CRS
        resampled = np.full(dem_shape, nodata_val, dtype=src.dtypes[0])  # fill with nodata_val initially
        ...
        # 3. Build new metadata aligned to DEM
        ...
        return resampled, nodata_val, new_meta
...
for idx, hz in enumerate(horizons):
    ...
    dem_file = os.path.join(morph_folder, "dem.asc")  # Add this line before the loop if not already defined

    bulkd, nodata_b, profile = read_clip_and_resample_raster(bulkd_path, shapes, dem_file, Resampling.bilinear)
    clay, nodata_c, _        = read_clip_and_resample_raster(clay_path, shapes, dem_file, Resampling.bilinear)
    sand, nodata_s, _        = read_clip_and_resample_raster(sand_path, shapes, dem_file, Resampling.bilinear)
...
for _, row in df_lookup.iterrows():
    line = f"{int(row['ID']):>9}{int(row['CLAY[%]']):>11}{int(row['SAND[%]']):>11}{row['Bd_mu[gcm-3]']:>18.2f}{int(row['nSamples']):>13}"
