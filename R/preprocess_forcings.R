  ...
  for (VAR in names(variables)) {
    ...
    if (fix.negatives){
      if (mhm_varnames[[VAR]] %in% c("pre","pet")){
        ...
        write_large_raster_to_netcdf(r.rs, final_path, varname = mhm_varnames[[VAR]], 
                                     batch_size,
                                     units,
                                     longname,
                                     time_origin,
      ...
      if (large.raster){
        ...
        writeCDF(r.rs, filename = final_path, overwrite = TRUE, prec = prec, missval = missval)
      }
    }
...
preprocess_LAI_data = function(domain_path, remove_temp = FALSE, iter_num = 10,
  ...
  # Fill no-data values
  # iter_num=10
  lai_filled <- focal_repeat(lai_clipped*0.1, n = iter_num)
...
preprocess_geo_data <- function(domain_path, remove_temp = FALSE, source.file = "local",
  ...
  if (source.file == "local"){
    ...
    resampled_path <- file.path(temp_folder, "geo_resampled.tif")
...
create_roi_mask = function(config_path, remove_temp = FALSE, basin.mask = FALSE){
  ...
  soil_file = file.path(morph_folder, "soil_class_horizon_01.asc")
  ...
  roi_mask = dem.m + geo.m + lc.m + lai.m + soil.m
